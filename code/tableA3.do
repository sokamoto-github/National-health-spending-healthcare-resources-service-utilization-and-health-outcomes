...
*---------------------------------------------------------------------------------------------------
	...
	*** The effects of health spending
	...
	cap erase $table/tableA3/tableA3.xls
	cap erase $table/tableA3/tableA3.txt	
	...
	* resources & utilization
	...
	foreach i in bed doc nurses care staff measles imr mr {
		...
		reghdfe ln`i' lnhealth logy `ctl', absorb(id year i.region#i.year)
		outreg2 using $table/tableA3/tableA3.xls, append drop(`ctl') auto(2) dec(2) nocon ci noaster addtext(Year Dummy, "Yes", Control, "Yes", Model, "FE") 
		}
		
		
	foreach i in bed doc nurses care staff measles imr mr {
		
		reghdfe ln`i' lnhealth logy `ctl', absorb(id year i.id#c.year i.id#c.yearsq)
		outreg2 using $table/tableA3/tableA3.xls, append drop(`ctl') auto(2) dec(2) nocon ci noaster addtext(Year Dummy, "Yes", Control, "Yes", Model, "FE") 
	}
