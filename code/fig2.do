...
*---------------------------------------------------------------------------------------------------
	...
	* income and government effectiveness
	...
	gen grp1990_ = incomegrp if year == 1990
	egen grp1990 = mean(grp1990_), by(id)	
	gen highgrp = .
	replace highgrp = 1 if grp1990 == 1 | grp1990 == 2
	replace highgrp = 2 if grp1990 == 3 | grp1990 == 4
	...
	foreach i in lnbed lndoc lnnurses lncare lnstaff lnmeasles lnimr lnmr{
		...
		reghdfe `i' lnhealth logy `ctl' if highgrp ==`n', absorb(id year)
			...
			parmest, saving($figure/fig2/`n'_`i'.dta, replace) idstr(`i') idnum(`n'`k')
		}
		}
...
*---------------------------------------------------------------------------------------------------
	...
	replace x = x -2 if row >= 15
		...
		label define xlabels    ///
				...
				8  " " ///
				7  "{it:B-1. High-income, Mortality}" ///	
				6  "13. Infant Mortality Rate" ///	
				5  "14. Mortality Rate" ///	
				4  "" ///	
				3  "{it:B-2. Low-income, Mortality}" ///
	...
	*** Figure employment
	twoway rspike  min95 max95 x if x >= 17, c(l) lc(blue*0.5) m(i) lstyle(solid) horizontal ///
		...
		xlab(-.4 (.2) .8, labsize(2.1) nogrid) xline(0, lp(shortdash) lw(thin) lc(gray)) legend(off) scheme(plotplain) ysize(10) xsize(10) ytitle("Subgroup and outcome variables", size(2.4)) xtitle("Estimated coefficients of Share of" "Total Health Spending in GDP", size(2.4))
		
		graph save "$figure/fig2/fig2" ,replace
		graph export "$figure/fig2/fig2.png", replace as(png) width(5000)
